...
class FloodRiskCalculator {
  ...
  FloodRisk calculate(RainfallData rainfall, String city) {
    ...
    if (rainfall.mm24h > 100) score += 35;
    else if (rainfall.mm24h > 50) score += 25;
    else if (rainfall.mm24h > 25) score += 15;
    else if (rainfall.mm24h > 10) score += 8;

    // Intensity contribution (0-25)
    if (rainfall.mmPerHour > 30) score += 25;
    else if (rainfall.mmPerHour > 20) score += 18;
    else if (rainfall.mmPerHour > 10) score += 10;
    else if (rainfall.mmPerHour > 5) score += 5;

    // 48h accumulation (soil saturation proxy) (0-10)
    if (rainfall.mm48h > 150) score += 10;
    else if (rainfall.mm48h > 75) score += 6;
    else if (rainfall.mm48h > 30) score += 3;

    score = score.clamp(0, 100);

    FloodRiskLevel level;
    if (score <= 25) level = FloodRiskLevel.low;
    else if (score <= 50) level = FloodRiskLevel.moderate;
    else if (score <= 75) level = FloodRiskLevel.high;
    else level = FloodRiskLevel.critical;
  ...
  String _buildExplanation(RainfallData r, int score) {
    final parts = <String>[];
    if (r.mm24h > 25) parts.add('${r.mm24h.toStringAsFixed(0)}mm in past 24h');
    if (r.mmPerHour > 10) parts.add('${r.mmPerHour.toStringAsFixed(0)}mm/hr intensity');
    if (r.mm48h > 75) parts.add('saturated soil from 48h accumulation');
