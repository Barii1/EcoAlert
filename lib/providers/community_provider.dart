...
class CommunityProvider extends ChangeNotifier {
    ...
    try {
      ...
      final results = await Future.wait([
        ...
        _db
            .from('reports')
            .select(
                'id, reporter_uid, reporter_name, hazard_type, details, location_label, image_urls, status, created_at')
            .eq('status', 'approved')
            .order('created_at', ascending: false)
            .limit(50),
      ]);
      ...
      final communityPosts = (results[0] as List)
          .map((m) =>
              CommunityPost.fromMap(Map<String, dynamic>.from(m), myUid: myUid))
          .toList();
  ...
  Future<bool> addPost({
    ...
    try {
      ...
      if (user == null) {
        _errorMessage = 'Sign in before posting.';
        return false;
      }
      ...
      try {
        ...
        debugPrint('[Community] profile lookup skipped: $e');
      }
      ...
      if (image != null) {
        try {
          ...
          imageUrl = _db.storage.from('community-images').getPublicUrl(path);
        } catch (e) {
      ...
      await _insertPostWithCompatibleSchema(payload);
      ...
      _errorMessage = 'Could not post: $e';
  ...
  Future<void> _insertPostWithCompatibleSchema(
    ...
    for (var attempt = 0; attempt <= optionalColumns.length; attempt++) {
      try {
        await _db.from('community_posts').insert(compatiblePayload);
        return;
      } catch (e) {
        final message = e.toString().toLowerCase();
        final missingColumn = optionalColumns.cast<String?>().firstWhere(
              (column) =>
                  column != null &&
                  compatiblePayload.containsKey(column) &&
        ...
        debugPrint(
          '[Community] Retrying post without missing column: $missingColumn',
        );
      }
    }
  }
  ...
  Future<void> toggleLike(String postId) async {
    try {
      ...
      await _db
          .from('community_posts')
          .update({'liked_by': updated}).eq('id', postId);
      await loadPosts();
    } catch (e) {
